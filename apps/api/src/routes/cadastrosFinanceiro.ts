...
const formaPagamentoSchema = z.object({
  codigo: z.string().min(1).max(30),
  nome: z.string().min(2).max(120),
  tipo: z.enum(['dinheiro', 'cartao_credito', 'cartao_debito', 'boleto', 'pix', 'transferencia', 'cheque', 'outro']).default('outro'),
  maxParcelas: z.coerce.number().int().min(1).max(999).default(1),
  taxaPercentual: z.coerce.number().min(0).max(100).default(0),
  prazoRecebimentoDias: z.coerce.number().int().min(0).default(0),
  empresaId: emptyToUndef(z.string().uuid().optional()),
  ativo: z.boolean().default(true),
})

app.get('/formas-pagamento', async (c) => {
  const tenant = c.get('tenant')
  const { busca } = c.req.query()
  const { limit, offset, page } = parseListPagination(c)
  ...
  const countRow = await c.env.DB_SHARED.prepare(`SELECT COUNT(*) as c FROM formas_pagamento${where}`).bind(...params).first<{ c: number }>()
  const { results } = await c.env.DB_SHARED.prepare(
    `SELECT * FROM formas_pagamento${where} ORDER BY codigo LIMIT ? OFFSET ?`,
  ).bind(...params, limit, offset).all()

  return c.json({ formas: results ?? [], total: Number(countRow?.c ?? 0), page, limit })
})

app.post('/formas-pagamento', zValidator('json', formaPagamentoSchema), async (c) => {
  const tenant = c.get('tenant')
  const body = c.req.valid('json')
  const dup = await c.env.DB_SHARED.prepare('SELECT id FROM formas_pagamento WHERE tenant_id = ? AND codigo = ?').bind(tenant.tenantId, body.codigo).first()
  if (dup) return c.json({ error: `Já existe uma forma de pagamento com o código "${body.codigo}".` }, 409)

  const id = crypto.randomUUID()
  const now = new Date().toISOString()
  await c.env.DB_SHARED.prepare(`
    INSERT INTO formas_pagamento (id, tenant_id, empresa_id, codigo, nome, tipo, max_parcelas, taxa_percentual, prazo_recebimento_dias, ativo, created_at, created_by)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(id, tenant.tenantId, body.empresaId ?? null, body.codigo, body.nome, body.tipo, body.maxParcelas, body.taxaPercentual, body.prazoRecebimentoDias, body.ativo ? 1 : 0, now, auditUserId(c)).run()

  return c.json({ id, message: 'Forma de pagamento cadastrada.' }, 201)
})

app.put('/formas-pagamento/:id', zValidator('json', formaPagamentoSchema.partial()), async (c) => {
  const tenant = c.get('tenant')
  const id = c.req.param('id')
  const body = c.req.valid('json')

  const row = await c.env.DB_SHARED.prepare('SELECT * FROM formas_pagamento WHERE id = ? AND tenant_id = ?').bind(id, tenant.tenantId).first()
  if (!row) return c.json({ error: 'Forma de pagamento não encontrada.' }, 404)

  if (body.codigo && body.codigo !== row.codigo) {
    const dup = await c.env.DB_SHARED.prepare('SELECT id FROM formas_pagamento WHERE tenant_id = ? AND codigo = ? AND id != ?').bind(tenant.tenantId, body.codigo, id).first()
    if (dup) return c.json({ error: `Já existe uma forma de pagamento com o código "${body.codigo}".` }, 409)
  }

  await c.env.DB_SHARED.prepare(`
    UPDATE formas_pagamento SET codigo = ?, nome = ?, tipo = ?, max_parcelas = ?, taxa_percentual = ?, prazo_recebimento_dias = ?, empresa_id = ?, ativo = ?, updated_at = ?, updated_by = ?
    WHERE id = ? AND tenant_id = ?
  `).bind(
    body.codigo ?? row.codigo, body.nome ?? row.nome, body.tipo ?? row.tipo,
    body.maxParcelas ?? row.max_parcelas, body.taxaPercentual ?? row.taxa_percentual, body.prazoRecebimentoDias ?? row.prazo_recebimento_dias,
    body.empresaId !== undefined ? body.empresaId : row.empresa_id,
    body.ativo !== undefined ? (body.ativo ? 1 : 0) : row.ativo,
    new Date().toISOString(), auditUserId(c), id, tenant.tenantId,
  ).run()

  return c.json({ message: 'Forma de pagamento atualizada.' })
})

app.delete('/formas-pagamento/:id', async (c) => {
  const tenant = c.get('tenant')
  const r = await c.env.DB_SHARED.prepare('DELETE FROM formas_pagamento WHERE id = ? AND tenant_id = ?').bind(c.req.param('id'), tenant.tenantId).run()
  if (!r.meta.changes) return c.json({ error: 'Forma de pagamento não encontrada.' }, 404)
  return c.json({ message: 'Forma de pagamento removida.' })
})
