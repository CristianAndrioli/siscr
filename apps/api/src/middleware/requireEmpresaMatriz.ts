...
export function allowsTenantAccessWithoutEmpresa(pathname: string, method: string): boolean {
  ...
  if (m === 'POST' && /^\/api\/tenant\/info\/empresas\/[^/]+\/certificado-a1\/atualizar-metadados$/.test(p)) return true

  if (m === 'POST' && /^\/api\/tenant\/info\/filiais\/[^/]+\/certificado-a1$/.test(p)) return true

  if (m === 'POST' && /^\/api\/tenant\/info\/filiais\/[^/]+\/certificado-a1\/atualizar-metadados$/.test(p)) return true
