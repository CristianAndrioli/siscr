import { useNavigate } from 'react-router-dom';
import {
  useErrorNotification,
  type ToastVariant,
} from '../../context/ErrorNotificationContext';

const VARIANT_UI: Record<
  ToastVariant,
  { border: string; icon: string; link: string }
> = {
  error: {
    border: 'border-red-200 dark:border-red-800',
    icon: 'text-red-500',
    link: 'text-red-500 hover:text-red-700 dark:hover:text-red-300',
  },
  warning: {
    border: 'border-amber-200 dark:border-amber-800',
    icon: 'text-amber-500',
    link: 'text-amber-600 hover:text-amber-800 dark:hover:text-amber-300',
  },
  success: {
    border: 'border-emerald-200 dark:border-emerald-800',
    icon: 'text-emerald-500',
    link: 'text-emerald-600 hover:text-emerald-800 dark:hover:text-emerald-300',
  },
  info: {
    border: 'border-sky-200 dark:border-sky-800',
    icon: 'text-sky-500',
    link: 'text-sky-600 hover:text-sky-800 dark:hover:text-sky-300',
  },
};

function ToastIcon({ variant }: { variant: ToastVariant }) {
  const cls = `w-5 h-5 ${VARIANT_UI[variant].icon}`;
  if (variant === 'success') {
    return (
      <svg className={cls} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
      </svg>
    );
  }
  if (variant === 'info') {
    return (
      <svg className={cls} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M11.25 11.25l.041-.02a.75.75 0 011.063.852l-.708 2.836a.75.75 0 001.063.853l.041-.021M21 12a9 9 0 11-18 0 9 9 0 0118 0zm-9-3.75h.008v.008H12V8.25z" />
      </svg>
    );
  }
  // error + warning
  return (
    <svg className={cls} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      <path
        strokeLinecap="round"
        strokeLinejoin="round"
        d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z"
      />
    </svg>
  );
}

/** Stack de toasts flutuantes (canto inferior direito). */
export default function ErrorToastStack() {
  const { notifications, dismiss, pauseDismiss, resumeDismiss } = useErrorNotification();
  const navigate = useNavigate();

  if (notifications.length === 0) return null;

  return (
    <div className="fixed bottom-5 right-5 z-[9999] flex flex-col gap-3 w-80 max-w-[calc(100vw-2rem)]">
      {notifications.map((n) => {
        const ui = VARIANT_UI[n.variant];
        return (
          <div
            key={n.id}
            className={`flex flex-col gap-2 bg-white dark:bg-slate-800 border ${ui.border} rounded-xl shadow-xl px-4 py-3 animate-slide-in`}
            role="status"
            onMouseEnter={() => pauseDismiss(n.id)}
            onMouseLeave={() => resumeDismiss(n.id)}
          >
            <div className="flex items-start gap-2">
              <span className="mt-0.5 flex-shrink-0">
                <ToastIcon variant={n.variant} />
              </span>
              <p className="flex-1 text-sm font-medium text-slate-800 dark:text-slate-100 leading-snug">
                {n.friendlyMessage}
              </p>
              <button
                type="button"
                onClick={() => dismiss(n.id)}
                className="flex-shrink-0 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-colors"
                title="Fechar"
              >
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                </svg>
              </button>
            </div>

            {n.logId && (
              <div className="flex justify-end">
                <button
                  type="button"
                  onClick={() => {
                    dismiss(n.id);
                    navigate(`/configuracoes/logs/${n.logId}`);
                  }}
                  className={`text-xs underline underline-offset-2 transition-colors ${ui.link}`}
                >
                  Ver detalhes do erro →
                </button>
              </div>
            )}
          </div>
        );
      })}
    </div>
  );
}
