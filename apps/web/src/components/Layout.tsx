...
const icons = {
  ...
  chevron: (open: boolean) => <path strokeLinecap="round" strokeLinejoin="round" d={open ? "M4.5 15.75l7.5-7.5 7.5 7.5" : "M19.5 8.25l-7.5 7.5-7.5-7.5"} />,
...
export default function Layout({ children }: LayoutProps) {
  ...
  // Lê dados do usuário do localStorage (salvo no login)
  ...
  const SidebarLink = ({ to, label, iconD }: { to: string; label: string; iconD: ReactNode }) => (
    ...
    >
      <span className={`flex-none ${isActive(to) ? 'text-brand-600' : 'text-slate-400'}`}>
        <Icon d={iconD} />
      </span>
      <span>{label}</span>
    </Link>
    ...
    <div>
      <button
        ...
        className={`flex items-center justify-between w-full px-3 py-2 rounded-lg text-sm font-medium transition-all duration-150 ${
          menuOpen[menuKey] ? 'text-slate-900 bg-slate-100' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100'
        }`}
      >
        <span className="flex items-center gap-3">
          <span className="flex-none text-slate-400"><Icon d={iconD} /></span>
          <span>{label}</span>
        </span>
        <svg className={`w-3.5 h-3.5 text-slate-400 transition-transform duration-200 ${menuOpen[menuKey] ? 'rotate-180' : ''}`}
          fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          {icons.chevron(false)}
        </svg>
      </button>
      {menuOpen[menuKey] && (
        <div className="ml-7 mt-0.5 border-l border-slate-200 pl-3 space-y-0.5">
          {sub}
        </div>
      )}
    </div>
  ...
  const sidebar = (
    <aside className="flex flex-col h-full bg-white border-r border-slate-200">
      {/* Logo */}
      <div className="flex items-center gap-3 px-5 h-14 border-b border-slate-100 flex-none">
        ...
        <div className="min-w-0">
          ...
          {tenantSlug && <div className="text-xs text-slate-400 mt-0.5 truncate">@{tenantSlug}</div>}
        </div>
      </div>
      ...
      {/* Usuário logado */}
      <div className="border-t border-slate-100 p-3 flex-none">
        <div className="flex items-center gap-2.5 px-2 py-2 rounded-lg hover:bg-slate-50 transition-colors group">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-xs text-slate-400 truncate">{tenantSlug && `@${tenantSlug}`}</div>
          </div>
          <button
            onClick={handleLogout}
            title="Sair"
            className="opacity-0 group-hover:opacity-100 w-7 h-7 rounded-md flex items-center justify-center text-slate-400 hover:text-red-500 hover:bg-red-50 transition-all"
          >
  ...
  return (
    <div className="flex min-h-screen bg-slate-50 font-sans">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Topbar */}
        <header className="sticky top-0 z-30 h-14 bg-white border-b border-slate-200 flex items-center gap-3 px-4 lg:px-6">
          <button
          ...
          {/* Badge tenant */}
          {tenantSlug && (
            <div className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-brand-50 border border-brand-100 text-brand-700 text-xs font-semibold">
