...
function ModuleCard({ label, sub, to, icon, layout, onClick }: {
  label: string; sub: string; to: string; icon: React.ReactNode
  layout: 'grid' | 'list'; onClick?: () => void
}) {
  ...
  if (layout === 'list') {
    return (
      <button
        onClick={handleClick}
        className="w-full flex items-center gap-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-brand-300 dark:hover:border-brand-700 rounded-xl px-4 py-3 transition-all text-left group"
      >
        ...
        <span className="text-sm font-semibold text-slate-800 dark:text-slate-100 group-hover:text-brand-700 dark:group-hover:text-brand-300 transition-colors">
          {label}
        </span>
        <svg className="w-4 h-4 text-slate-300 dark:text-slate-600 group-hover:text-brand-400 ml-auto flex-none transition-all group-hover:translate-x-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
  ...
  return (
    <button
      onClick={handleClick}
      className="flex flex-col gap-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-brand-300 dark:hover:border-brand-700 hover:shadow-sm rounded-xl p-4 transition-all text-left group"
    >
      <div className="w-10 h-10 rounded-xl bg-brand-50 dark:bg-brand-950 text-brand-600 dark:text-brand-400 flex items-center justify-center flex-none group-hover:bg-brand-100 dark:group-hover:bg-brand-900 transition-colors">
        {icon}
      </div>
      <div>
...
function AppHome() {
  ...
  const handleModuleClick = (mod: typeof ALL_MODULES[0]) => {
    trackRecentItem({ label: mod.label, to: mod.to, icon: mod.key })
  }
