...
export default function CentrosCustoPage() {
  ...
  const carregar = useCallback(async () => {
    ...
    setError('');
    try {
      const r = await centrosCustoService.list({ search: appliedSearch || undefined, limit: 200 });
      setCentros(r.centros);
      setTotal(r.total);
    } catch (e) {
      setError(formatApiError(e, 'Erro ao carregar centros de custo.'));
    } finally {
      setLoading(false);
    }
  }, [appliedSearch]);
  ...
  const handleDelete = async () => {
    if (!deleting) return;
    try {
      await centrosCustoService.delete(deleting.id);
      setSuccess('Centro de custo removido.');
      setDeleting(null);
      await carregar();
    } catch (e) {
      setError(formatApiError(e, 'Erro ao remover centro de custo.'));
      setDeleting(null);
    }
  };
  ...
  return (
    <BaseListPage
      ...
      description="Estrutura de centros de custo para apuração e rateio de despesas"
      badge="NOVO CADASTRO"
      onExport={() => exportRowsToCsv('centros-custo', [
        { label: 'Código', value: (r: CentroCusto) => r.codigo },
        { label: 'Nome', value: (r: CentroCusto) => r.nome },
        { label: 'Ativo', value: (r: CentroCusto) => (r.ativo === 1 ? 'Sim' : 'Não') },
      ], centros)}
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        >
          <input
            ...
            placeholder="Buscar em centros de custo…"
      ...
      {loading ? (
        <LoadingSpinner fullScreen text="Carregando centros de custo..." />
      ) : centros.length === 0 ? (
      ...
      >
        <p className="text-sm text-slate-600 dark:text-slate-400">
          Remover o centro de custo <strong>{deleting?.nome}</strong>? Esta ação não pode ser desfeita.
        </p>
      </Modal>
    </BaseListPage>
  );
