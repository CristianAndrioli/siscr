    ...
    <div className="max-w-4xl space-y-6">
      ...
      {info && (
        <div className="bg-emerald-50 dark:bg-emerald-950 border border-emerald-200 dark:border-emerald-800 text-emerald-700 dark:text-emerald-300 px-4 py-3 rounded-lg text-sm flex items-start gap-2">
          ...
          <span>
            {info}
            {' '}<button onClick={() => navigate(infoNotaId ? `/faturamento/nfse/${infoNotaId}` : '/faturamento/nfse')} className="underline font-medium ml-1">Ver NFS-e em Faturamento →</button>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {medicoes.map(m => (
                    <tr key={m.id} className="text-slate-700 dark:text-slate-200">
                      ...
                      <td className="px-4 py-2 text-right">
                        {m.nota_fiscal_id && <button onClick={() => navigate(`/faturamento/nfse/${m.nota_fiscal_id}`)} className="text-brand-600 dark:text-brand-400 hover:underline text-xs font-medium">Ver NFS-e</button>}
                      </td>
                    </tr>
                  ))}
                </tbody>
